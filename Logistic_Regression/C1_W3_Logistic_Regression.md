...
value = 0
print(f"sigmoid({value}) = {sigmoid(value)}")  # Expected output: 0.5

print("sigmoid([ -1, 0, 1, 2]) = " + str(sigmoid(np.array([-1, 0, 1, 2]))))
# Expected output: [0.26894142 0.5 0.73105858 0.88079708]
...
def compute_cost(X, y, w, b, *argv):
    ...
    for i in range(m):
        z_wb = np.dot(X[i], w) + b   # Compute the linear combination
        f_wb = sigmoid(z_wb)         # Apply the sigmoid function
        cost_i = -y[i] * np.log(f_wb) - (1 - y[i]) * np.log(1 - f_wb) # Compute loss for the i-th example
        total_cost += cost_i         # Accumulate the total cost

    total_cost = total_cost / m      # Average cost over all examples
...
def compute_gradient(X, y, w, b, *argv): 
    ...
    ### START CODE HERE ### 
    for i in range(m):
        # Calculate the linear combination z_wb = w * x + b
        z_wb = np.dot(X[i], w) + b
        
        # Calculate the model's prediction f_wb using the sigmoid function
    ...
    # Divide by m to get the average gradients
    dj_dw /= m
    dj_db /= m
    ### END CODE HERE ###
...
def map_feature(x1, x2):
    ...
    """
    # Add polynomial features (e.g., x1^2, x2^2, x1*x2) for this example
    # This is a placeholder; adjust as needed for your data
...
# UNIT TEST
def compute_cost_reg_test(func):
    try:
        func(X_mapped, y_train, initial_w, initial_b, lambda_)
...
```
    ...
    Regularized cost: 7.47639968381221
